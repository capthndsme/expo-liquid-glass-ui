...
| `refraction.swirl` | `number` | Android | How far the edge refraction leans toward `highlight.angle`'s light axis, unitless like `depth`. Default `0.25`; `0` restores the pure normal+radial direction. Real iOS 26 glass twists its edge refraction toward the light — more `amount`, more twist — and this is that twist. Negative flips it; clamped to `[-1, 1]`. iOS drops the key. |
