...
| `highlight.width` | `number` | Android | Depth of the crisp border-light line, in dp. Default `0.75` — Apple's line measures 2–3 px on a 238 px icon. A separate faint ~7 dp sheen under the lit edges and a 1.5 pt multiplicative dark contour on the non-lit flanks ride the same lobes at fixed ratios. All of it replaces the Metal fallback's `refraction.height`-wide wash, which multiplied (so it vanished over dark backdrops), lit one lobe only, and faked an inset shadow real glass does not have. iOS drops the key. |
